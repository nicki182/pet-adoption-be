...
class AnimalServices{
    ...
    public async getAnimalByField(
        ...
        const animalData: PrismaClient.Animal = await this.prisma.getByField(field, value, select) as PrismaClient.Animal;
        const user = await UserServices.getUserByField('id', String(animalData.userId)) ;
        return new Animal({
            id: animalData.cuid,
    ...
    public async updateAnimal(
        ...
        const user = await UserServices.getUserByField('id', String(updatedAnimal.userId)) ;
        return new Animal({
    ...
    public async createAnimal(
        ...
        const user = await UserServices.getUserByField('id', String(createdAnimal.userId)) ;
        return new Animal({
    ...
    public async getAllAnimals(
        ...
        return Promise.all(animals.map(async (animal: PrismaClient.Animal) => {
            const user = await UserServices.getUserByField('id', String(animal.userId)) ;
            return new Animal({
                id: animal.cuid,
                name: animal.name,
                age: animal.age,
