...
class ControllerSession {
    async logInSession(data:DataRequest):Promise<Session> {
        try{
        const user:UserModelI = await UserServices.getByField('email',data.email);
        if(!user) throw new CustomError("User not found")
        if(!UserServices.validatePassword(data.password,user.password)) throw new Error("Password is incorrect")
