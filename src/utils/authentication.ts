...
async function validateSchema<T extends IValidationSchema,I extends Record<string,unknown>> (schema:ObjectSchema<T>, data:I) {
  try{
    ...
    return {errors:_.get(error,'errors',[]),values:_.get(error,'values',{})}
  }
}

export { generateToken, verifyToken, hashPassword, comparePassword, validateSchema,decodeToken };
