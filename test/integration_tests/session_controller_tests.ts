...
describe("Session Controller tests", () => {
    ...
    it("Session refresh token", () => {
        ...
        Controller.signUpSession(data).then((session)=>{
            setTimeout(() => {
                SessionService.getSession(session.getUserId()).then((session) => {
                    Controller.refreshAccessToken(session.getUserId(),session.getRefreshToken()).then((session) => {
    ...
    it("Session is not logged out by refresh token", () => {
        ...
        Controller.signUpSession(data).then((session)=>{
            setTimeout(() => {
                SessionService.getSession(session.getUserId()).then((session) => {
                    Controller.refreshAccessToken(session.getUserId(),session.getRefreshToken())
                })
            },100);
            setTimeout(() => {
                SessionService.getSession(session.getUserId()).then((session) => {
    ...
    it("Session log in", () => {
        ...
        UserServices.createUser(data,{cuid:true}).then(() => {
            Controller.logInSession({email:data.email,password:data.password}).then((session) => {
                expect(session).to.be.a("object");
            })
        })
    })
