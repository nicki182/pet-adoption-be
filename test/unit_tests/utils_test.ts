...
import { generateToken,verifyToken,validateSchema,hashPassword,comparePassword,decodeToken } from "../../src/utils/authentication";
...
it("Validate data is true",async ()=>{
  ...
  expect(await (await validateSchema(schema,data))).to.deep.equal({errors:[],values:data})
}),
it("Validate data is false",async ()=>{
  ...
  expect(await (await validateSchema(schema,data))).to.deep.equal({errors:["Name"],values:data})
})
